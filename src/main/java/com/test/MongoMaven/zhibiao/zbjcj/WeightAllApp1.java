...
//				System.out.println(map.toString());
				//写数据库
					...
					MongoCollection<Document> collection=mongo.getShardConn("xg_all_app_combine");
...
//						HashMap<String, Object> result=entry.getValue();
						...
						for(int i=1;i<=size;i++){
							...
							if(map.containsKey(name)){
								...
								tweight=tweight+Float.parseFloat(val);
								si++;
							}
						}
						if(si>0){
							String c=df.format((float)tweight/si);//准确度
							doc.remove("_id");
							doc.put("weight", c);
							mongo.upsertDocByTableName(doc, "app_xg_all_our1");
						}
					}
					cursor.close();
			}
