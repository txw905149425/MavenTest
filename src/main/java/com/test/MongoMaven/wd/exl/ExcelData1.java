...
//	        title.add("000972");
	        ...
	        for (String  code:title){
	            ...
	            sheet1.addMergedRegion(new CellRangeAddress(0,0,i,i+4)); //	          	表示将first row, last row,first column,last column
	            for(int z=0;z<5;z++){
...
//		            		String name="jcj";
	        ...
	        FileOutputStream fileOut = new FileOutputStream("d:/ss_data/ss_data2.xlsx");  
	        wb.write(fileOut);  
	        fileOut.close();
	}
	...
	public static ArrayList<HashMap<String , String>> getComments(String code){
		...
		while(cursor.hasNext()){
			...
			for(int i=1;i<=num;i++){
				...
				String np=IKFunction.keyVal(one,"np").toString();
				String rose="";
