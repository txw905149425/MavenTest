...
public class ComplieStockFor1 {
	...
	public static void main(String[] args) {
		try{
		mongo.getShardConn("xg_zhibiao_all_web").deleteMany(Filters.exists("id"));
		ArrayList<String> list=FileUtil.readFileReturn("/home/jcj/crawler/xg_bat/buyao");
//		ArrayList<String> list=FileUtil.readFileReturn("buyao");
		for(String table:list){
			insert2Table(table);
		}
		complie2Table("xg_zhibiao_all_web");
		}catch(Exception e){
			e.printStackTrace();
		}
	}
		 ...
		 while(cursor.hasNext()){
			 ...
			 if(doc.containsKey("list")){
				 ...
				 for(int i=1;i<=num;i++){
					 ...
					 mongo.upsertMapByTableName(records,"xg_zhibiao_all_web");
				 }
			 }else{
...
//			                return t2.compareTo(t1);
			   ...
			   mongo.upsetManyMapByTableName(list, "xg_zhibiao_all_web_last");
			   mongo.upsetManyMapByTableName(list, "xg_zhibiao_all_web_last_all");
			} catch (Exception e) {
