...
public class XiaoquName {
	 public static void main(String[] args) {
			...
			HashMap<String , String> map =new HashMap<String, String>();
			 ...
			 String furl="https://www.anjuke.com/beijing/cm/p1/";
			...
			for(int i=0;i<num;i++){
				String tmp=IKFunction.jsoupListAttrByDoc(doc, ".p2a", "href", i).trim()+"p";
				 for(int j=1;j<35;j++){
					 ...
					 try {
						mongo.upsetManyMapByTableName(list, "ajk_beijing_community_name");
					} catch (Exception e) {
	 ...
	 public static  List<HashMap<String, Object>>  parseList(String html){
			...
			for(int i=0;i<num;i++){
				...
				String url="http://beijing.anjuke.com/community/view/"+uid;
				map.put("id", name);
				map.put("url", url);
				map.put("uid", uid);
				list.add(map);
