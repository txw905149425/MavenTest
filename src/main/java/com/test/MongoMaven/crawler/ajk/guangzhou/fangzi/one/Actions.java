package com.test.MongoMaven.crawler.ajk.guangzhou.fangzi.one;
	...
	public void run() {
		...
		try {
			...
			if(!StringUtil.isEmpty(html)&&htmlFilter(html,"p.title>span")){
				Object tdoc=IKFunction.JsoupDomFormat(html);
				String content=IKFunction.jsoupTextByRowByDoc(tdoc, ".m-noHouse>p", 0);
			if(!content.contains("该小区暂时没有在租房源")){
					...
					String xml=HttpUtil.getHtml(durl.toString(), new HashMap<String, String>(), "utf8", 1,applyProxy()).get("html");
						if(!StringUtil.isEmpty(xml)&&ParseMethod.htmlFilter(xml, ".wrapper>div>h3")){
							 ...
							 HashMap<String, Object > map=new HashMap<String, Object>();
								map.put("id", url);
								map.put("crawl_all", "1");
								mongo.upsertMapByTableName(map, "ajk_guangzhou_list_url");
