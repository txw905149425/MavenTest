package com.test.MongoMaven.crawler.ajk.guangzhou.fangzi.durl;
...
import com.test.MongoMaven.crawler.ajk.guangzhou.fangzi.one.ParseMethod;
 ...
 * 新浪股吧
 * */
public class GuangzhouCrawleByDurl {
	
	public static void main(String[] args) {
		 MongoDbUtil mongo=new MongoDbUtil();
		 MongoCollection<Document>  collection=mongo.getShardConn("ajk_guangzhou_detail_url");
			 ...
			 while(cursor.hasNext()){
				 ...
				 if(resultmap.isEmpty()){
					...
					if(!StringUtil.isEmpty(html)&&ParseMethod.htmlFilter(html, ".wrapper>div>h3")){
						 ...
						 records.put("url", url);
						 records.put("uid",uid);
						 mongo.upsertMapByTableName(records, "ajk_guangzhou_house_information");
						 HashMap<String, Object > map=new HashMap<String, Object>();
						 map.put("id", url);
						 map.put("crawl", "1");
						mongo.upsertMapByTableName(map, "ajk_guangzhou_detail_url");
