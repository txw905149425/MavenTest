package com.test.MongoMaven.crawler.ajk.shanghai.fangzi.one;
				...
				for(HashMap<String, Object > listmap:list){
					...
					String xml=HttpUtil.getHtml(durl.toString(), new HashMap<String, String>(), "utf8", 1,applyProxy()).get("html");
						if(!StringUtil.isEmpty(xml)&&ParseMethod.htmlFilter(xml, ".wrapper>div>h3")){
							 ...
							 records.put("uid",uid);
							 mongo.upsertMapByTableName(records, "ajk_shanghai_house_information");
							 HashMap<String, Object > map=new HashMap<String, Object>();
								map.put("id", url);
								map.put("crawl_all", "1");
								mongo.upsertMapByTableName(map, "ajk_shanghai_list_url");
