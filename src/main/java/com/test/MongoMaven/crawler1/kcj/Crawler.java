...
//		String url="http://gmv.cjzg.cn/Mv/get_more.html";
	...
	public static List<HashMap<String, Object>> parseList(String html){
		...
		for(int i=1;i<=num;i++){
			...
			String  scode=IKFunction.keyVal(stock, "code").toString();
			if(!question.contains(sname)&&!question.contains(scode)){
				question="["+sname+"("+scode+")]"+question;
			}else{
				question=question.replaceAll("<.*>", "["+sname+"("+scode+")]");
			}
