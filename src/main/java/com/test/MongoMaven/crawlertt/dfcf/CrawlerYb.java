...
//		 Bson filter = Filters.eq("id", "600499");
	  ...
	  try{
		 while(cursor.hasNext()){
			 ...
			 String url="http://datainterface.eastmoney.com//EM_DataCenter/js.aspx?type=SR&sty=GGSR&ps=25&p=1&code="+code;
...
//				 System.out.println(html);
	...
	public static void parseList(String html,String code,String name){
		Object json=IKFunction.arrayFmt(html);
		int num=IKFunction.rowsArray(json);
	 try{
		 List<HashMap<String, Object>> list1=new ArrayList<HashMap<String,Object>>();
			...
			map1.put("code", code);
			list1.add(map1);
		 String tt=IKFunction.getTimeNowByStr("yyyyMMdd");
		for(int i=1;i<=num;i++){
			Object one=IKFunction.array(json, i);
			String timeObj=IKFunction.keyVal(one, "datetime").toString();
			String time=timeObj.replace("T", " ");
			if(!IKFunction.timeOK(time)){
				break;
			}
			Object title=IKFunction.keyVal(one, "title");
			Object str=IKFunction.keyVal(one, "infoCode");
//			String url="http://data.eastmoney.com/report/20170628/APPHjRYcBjkIASearchReport.html";
			String url="http://data.eastmoney.com/report/"+tt+"/"+str+".html";
			String dhtml=HttpUtil.getHtml(url, new HashMap<String, String>(), "utf8", 1, new HashMap<String, String>()).get("html");
			 if(!StringUtil.isEmpty(dhtml)&&IKFunction.htmlFilter(dhtml, ".newsContent")){
				 HashMap<String, Object > map= parseDetail(dhtml);
				map.put("id", IKFunction.md5(title+"东方财富"+code));
				map.put("timedel", IKFunction.getTimeNowByStr("yyyy-MM-dd"));
				map.put("tid", title);
				map.put("newsClass", "研报");
				map.put("source", "东方财富");
				map.put("title", title);
				map.put("time", time);
...
//				mongo.upsertMapByTableName(map, "tt_dfcf");
	...
	public static HashMap<String, Object > parseDetail(String html){
		...
		String content=IKFunction.jsoupTextByRowByDoc(doc, ".newsContent", 0);
